...
#### Step 5: Regression analysis between FHOD3 genotype data and the filtered (104 samples of) phenotype data ####

out_all<-matrix(NA,ncol(e_f),7) #define a matrix comprising 7 columns and rows matching the numer of rows in e_f

i=3 #for specified FHOD3 genotype column
...
#{
	for(j in 3:ncol(e_f)) #and for every other filtered phenotype column
	{
		...
		p<-out1$coefficients["FHOD3",4] #select the p value from the summary statistics
		beta<-out1$coefficients["FHOD3",1] #select the beta value from the summary statistics
...
#phenotypePlot(out_plot,max.y=9,suggestive.line=-log10(1e-05), significant.line=-log10(5e-08), title="FHOD3",use.color=F,x.group.labels=F)
#dev.off()